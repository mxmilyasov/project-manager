...
manager-init: manager-composer-install manager-wait-db manager-migration

manager-composer-install:
	docker-compose run --rm manager-php-cli composer instal
...
deploy-production:
	...
	ssh -o StrictHostKeyChecking=no ${PRODUCTION_HOST} -p ${PRODUCTION_PORT} 'echo "MANAGER_DP_PASSWORD=${MANAGER_DP_PASSWORD}" >> .env'
